...
public class HibernateTemplate extends HibernateAccessor implements HibernateOperations {
    ...
    protected <T> T doExecute(HibernateCallback<T> action, boolean enforceNewSession, boolean enforceNativeSession)
        ...
        // 这里是取得Hibernate的Session，取得的过程会在下面进行分析
        // 判断是否强制需要新的Session，如果需要，那么直接通过SessionFactory
        // 打开一个新的Session，否则需要结合配置和当前的Transaction的情况来使用。
        Session session = (enforceNewSession ?
                SessionFactoryUtils.getNewSession(getSessionFactory(), getEntityInterceptor()) : getSession());
        boolean existingTransaction = (!enforceNewSession &&
                (!isAllowCreate() || SessionFactoryUtils.isSessionTransactional(session, getSessionFactory())));
        // 判断Transaction是否已经存在，如果是，那么使用的就是当前Transaction的session
        ...
        try {
            ...
            // 这里是对HibernateCallback中的回调函数的调用，Session作为参数可以由函数使用
            ...
            // 如果存在Transaction，那么当前的回调使用完Session后，不关闭这个Session
            if (existingTransaction) {
                ...
                // 如果不存在Transaction，那么关闭当前的Session
