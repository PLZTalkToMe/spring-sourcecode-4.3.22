...
public class LocalSessionFactoryBean extends AbstractSessionFactoryBean implements BeanClassLoaderAware {
    ...
    protected SessionFactory buildSessionFactory() throws Exception {
        ...
        // 创建SessionFactory的配置对象
        Configuration config = newConfiguration();

        // 配置数据源、事务管理器、和LobHandler到Holder中，这些Holder是ThreadLocal变量，
        // 这样这些资源就和线程绑定了。
        ...
        // Hibernate不允许直接设置Bean的ClassLoader，需要通过线程的方式实现
		...
		// 很长度配置清单，对Hibernate的各个属性进行配置，这里通过Hibernate的Configuration
        // 来实现配置。
        try {
            ...
            if (dataSource != null) {
                ...
                // 使用Spring提供的数据源，设置Hibernate的ConnectionProvider
            ...
            // 这里是Configuration配置创建SessionFactory的地方
     ...
     *
     *
     * 调用Configuration生成SessionFactory
     *
     *
