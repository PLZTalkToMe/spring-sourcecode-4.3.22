...
public class SqlFunction<T> extends MappingSqlQuery<T> {
     ...
     *
     * 对于SqlFunction而言，super.findObject(parameters);
     * 方法定义了SingleColumnRowMapper来完成数据记录到数据对象的转换，
     * 而不需要应用去实现这个转换的具体设计。这个数据转换器SingleColumnRowMapper是Spring定义好的
     * 其作用与使用MappingSqlQuery对象时mapRow方法的作用一样，
     * 即完成数据库数据查询记录到Java数据对象的转换的实现
     *
