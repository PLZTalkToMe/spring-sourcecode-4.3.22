...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
    ...
    public final TransactionStatus getTransaction(TransactionDefinition definition) throws TransactionException {
        ...
        if (definition == null) {
            ...
            // 如果没有给定任何事务处理定义就是用默认定义
            definition = new DefaultTransactionDefinition();
        }


        // 检查当前线程是否已经存在事务，如果已经存在事务，那么需要根据在事务属性中定义的事务传播属性
        // 配置来处理事务的产生。
        if (isExistingTransaction(transaction)) {
            // Existing transaction found -> check propagation behavior to find out how to behave.
            // 这里对当前线程已经有事务存在的情况进行处理吗，结果封装在TransactionStatus中
            return handleExistingTransaction(definition, transaction, debugEnabled);
        }

        // Check definition settings for new transaction.
        // 检查事务属性timeout的设置是否合理
        if (definition.getTimeout() < TransactionDefinition.TIMEOUT_DEFAULT) {
            throw new InvalidTimeoutException("Invalid transaction timeout", definition.getTimeout());
        }

        // No existing transaction found -> check propagation behavior to find out how to proceed.
        // 当前没有事务的存在，这时候需要根据事务属性设置来创建事务
        // 这里会看到对事务传播属性设置的处理，比如mandatory、require、require_new、nested等
        ...
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
            ...
            try {
                // 这里是创建事务的调用，由具体的处理器来完成，比如HibernateTransactionManager
    ...
    private TransactionStatus handleExistingTransaction(
        ...
        // 说明这种情况时有问题的。spring无法处理当前的事务创建。
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
            throw new IllegalTransactionStateException(
                    "Existing transaction found for transaction marked with propagation 'never'");
        }

        // 如果当前事务的配置属性是PROPAGATION_NOT_SUPPORTED，同事当前线程已经存在事务，那么将事务挂起。
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
            ...
            boolean newSynchronization = (getTransactionSynchronization() == SYNCHRONIZATION_ALWAYS);
            // 注意这里的参数，transaction为null，newTransaction为false
            // 意味着事务事务方法不需要放在事务环境中执行
            // 同时挂起事务的信息记录也保存在TransactionStatus中，这里
            // 包括了进程ThreadLocal对事务信息的记录。
            return prepareTransactionStatus(
                    definition, null, false, newSynchronization, debugEnabled, suspendedResources);
        }

        // 如果当前事务的配置属性是PROPAGATION_REQUIRES_NEW，创建新事务，同时把当前线程中存在的事务挂起
        // 与创建全新事务的过程类似，区别在于，在创建全新事务时，
        // 不用考虑已有事务的挂起，但在这里，需要考虑已有事务的挂起处理
        ...
        // 这里判断当前事务方法中的属性配置与已有事务的属性配置是否一致，如果不一致，
        // 那么不执行事务方法，并抛出异常。
        ...
        // 返回TransactionStatus，注意这里的第三个参数为false，
        // 代表当前事务方法没有开启新的事务。
        boolean newSynchronization = (getTransactionSynchronization() != SYNCHRONIZATION_NEVER);
     ...
     *
     *
     * 返回的SuspendedResourcesHolder将作为参数传给TransactionStatus
     *
     *
     *
    ...
    protected final SuspendedResourcesHolder suspend(Object transaction) throws TransactionException {
        if (TransactionSynchronizationManager.isSynchronizationActive()) {
            ...
            try {
                ...
                // 把挂起事务的处理交给具体的事务处理器去完成，如果具体的事务处理器不支持事务挂起
                if (transaction != null) {
                    suspendedResources = doSuspend(transaction);
                }
                // 这里在线程中保存与事务处理有关的信息，并重置线程中有关的ThreadLocal变量
        ...
        try {
            ...
            try {
                ...
                if (status.hasSavepoint()) {
                    ...
                    // 下面对根据当前线程中保存的事务状态进行处理，如果当前事务是一个新的事务
                    // 调用具体的事务处理器完成提交，如果当前所持有的事务不是一个新的事务，则不提交
                    // 由已经存在的来完成提交。
                    if (status.isDebug()) {
